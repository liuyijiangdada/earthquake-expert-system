#!/usr/bin/env python3
"""按导师意见改稿方案重写《华东师范大学硕士论文.docx》正文。

保留封面、声明、摘要标题结构；替换摘要正文与关键词；
从「第一章」起至「致谢」前重建第1–7章、参考文献与附录。
...
OUT_DEFAULT = ROOT / "华东师范大学硕士论文_导师意见修改版.docx"
RED = RGBColor(0xFF, 0x00, 0x00)

import sys

sys.path.insert(0, str(ROOT))
from scripts.thesis_rewrite_content import (  # noqa: E402
    ABSTRACT_CN,
    ABSTRACT_EN,
    APPENDIX_BLOCKS,
    BODY_BLOCKS,
...
def _format_para(para: Paragraph, level: str, *, mark_red: bool = True) -> None:
    ...
    elif level in ("caption", "table_title"):
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        pf.first_line_indent = Cm(0)
...
def apply_content(
    ...
    *,
    ...
    # prefer the 致谢 after body (last occurrence)
    for i, p in enumerate(doc.paragraphs):
        if p.text.strip() in ("致谢",) and i > body_start:
            ack_i = i

    # delete from body_start inclusive to ack_i exclusive
    ...
    # insert new content before 致谢: walk reverse so order preserved with insert_before
    # easier: insert after a marker placed just before 致谢
    # Create empty anchor para before 致谢
    ...
    # append references + appendix into stream
    ...
    # remove the initial empty paragraph we created (if still empty and precedes 第一章)
    ...
    # update TOC block (between 目录 and 图目录 / 第一章 body was deleted; TOC remains)
    try:
        toc_i = _find_para_index(doc, lambda t: t == "目录")
    except KeyError:
        return

    # wipe old TOC entries until 图目录 or 表目录 or 第一章 body-ish
