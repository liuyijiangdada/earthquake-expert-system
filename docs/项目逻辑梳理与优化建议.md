...
sequenceDiagram
    ...
    API-->>UI: response + meta
```

| 步骤 | 位置 | 说明 |
|------|------|------|
| 1 | `frontend/src/api.js` → `queryLlm()` | `POST /api/query`，`params.input` |
| 2 | `App.vue` → `sendMessage()` | 60s 超时；展示 `meta.phase`、`media_resources` |
...
| 7 | `multimodal_output.match_as_dicts()` | **仅写入 meta**，不进 prompt |
...
| 6 | `multimodal_output` | 结果进 `meta.media_resources` |
...
| 第 3 周 | VL/文本内存互斥或 VL CPU + meta 开关 | 图片问答稳定 |
